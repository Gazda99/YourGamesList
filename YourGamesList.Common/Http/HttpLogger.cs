...
public class HttpLogger : IHttpClientLogger
{
        ...
        object? context, HttpRequestMessage request, HttpResponseMessage response, TimeSpan elapsed)
    {
        ...
        {
            _logger.LogInformation(
                $"Received '{(int) response.StatusCode} {response.StatusCode}' after {elapsed.TotalMilliseconds:F1}ms");
        }
    }
    ...
    private static Dictionary<string, object> RequestFailedScopes(HttpRequestMessage request, TimeSpan elapsed)
    {
        return new Dictionary<string, object>
        {
            ["Request.Method"] = request.Method,
            ["Request.Host"] =
                request.RequestUri?.GetComponents(UriComponents.SchemeAndServer, UriFormat.Unescaped) ?? "N/A",
            ["Request.Path"] =
